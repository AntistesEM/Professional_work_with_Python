...
def logger(path):
    ...
    def __logger(old_function):
        def new_function(*args, **kwargs):
            start = datetime.now().strftime('%Y-%m-%d время: %H:%M:%S:%f00')
            name_function = old_function.__name__
            arguments = (
                ', '.join([str(arg) for arg in args]) if args else 'args нет'
            )
            kwarguments = (
                ', '.join(f'{key}={value}' for key, value in kwargs.items())
                if kwargs else 'kwargs нет'
            )
            results = old_function(*args, **kwargs)
            with open(full_path + path, 'a', encoding='utf-8') as file:
                file.write(f'Время запуска: {start}\n'
